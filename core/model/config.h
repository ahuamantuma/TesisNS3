 ...
 * \param [in] value The value to get in all matching attributes.
 *
 * This function will attempt to find attributes which
 * match the input path and will then get their value to the input
 * value. Only for the Tap Bridge API.
 */
...
public:
   ...
   * Added by AH:
   *
   * \param [in] name Name of attribute to get
   * \param [in] value Value to get to the attribute
   *
   * Get the specified attribute value to all the objects stored in this
   * container.
   * \sa ns3::Config::Get
   */
  void GetAttribute (std::string name, AttributeValue &value);
