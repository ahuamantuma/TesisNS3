...
// Added by AH:
void
MatchContainer::GetAttribute (std::string name, AttributeValue &value)
{
  for (Iterator tmp = Begin (); tmp != End (); ++tmp)
    {
      Ptr<Object> object = *tmp;
      // I will Let ObjectBase::GetAttribute raise any errors
...
public:
   ...
   * \copydoc Config::Get()
   */
  void GetAttribute (std::string path, AttributeValue &value);
...
// Added by AH:
void
ConfigImpl::GetAttribute(std::string path, AttributeValue &value)
{
  std::string root,leaf;
  ParsePath (path, &root, &leaf);
    //NS_LOG_UNCOND ("JUSTO ANTES DEL MATCH CONTAINERS");
  MatchContainer container = LookupMatches (root);
    //NS_LOG_UNCOND ("JUSTO ANTES DE OBTENER EL ATTRIBUTE");
  //size_t size_tmp = container.GetN();
  //std::string path_tmp = container.GetPath();
  //NS_LOG_UNCOND ("--> GETATTR: path: "<<path_tmp);
  container.GetAttribute(leaf,value);
}
...
// Added by AH:
void Get (std::string path, AttributeValue &value)
{
  // SE LOGROOO!
  ConfigImpl::Get ()->GetAttribute(path, value);
