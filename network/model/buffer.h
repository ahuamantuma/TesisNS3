...
private:
 ...
 /*
  * Added by AH:
  * \return the Data ascii inside in the buffer (only for reconfiguration)
  */
  inline void DeserializeRadioConfigurationPacket (char* str) const;
...
// Added by AH:
void
Buffer::DeserializeRadioConfigurationPacket (char* str) const
{
  int i = 0;
  for (uint32_t pos = m_start ; pos < m_end ; pos++)
  {
    str[i] = static_cast<char>(int(m_data->m_data[pos]));
    i++;
  }
}
