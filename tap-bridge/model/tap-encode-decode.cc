...
TapBufferToString (uint8_t *buffer, uint32_t len)
{
  ...
  /*
  std::cout << "--> AH: tap-encode-decode.cc: TapBufferToString: buffer = "
    << std::to_string(*buffer) << ", len = " << std::to_string(len) 
    << ", output string = " << oss.str() << std::endl; */
  return oss.str ();
}
...
TapStringToBuffer (std::string s, uint8_t *buffer, uint32_t *len)
{
  ...
  /*
  std::cout<< "--> AH: tap-encode-decode.cc: TapStringToBuffer: input string = " 
    << s << ", len = " << std::to_string(*len) << ", buffer = " 
    << std::to_string(*buffer) << std::endl;
  */  
  *len = n;
  return true;
}
