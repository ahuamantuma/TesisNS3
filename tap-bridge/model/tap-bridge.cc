...
FdReader::Data TapBridgeFdReader::DoRead (void)
{
  ...
  //NS_LOG_UNCOND ("--> AH: TapBridgeFdReader::DoRead -> pkt len: "
  //    << std::to_string(len));
...
TapBridge::ForwardToBridgedDevice (uint8_t *buf, ssize_t len)
{
  ...
  //NS_LOG_UNCOND("--> AH: El EtherType es en decimal: "<< type);
  
  // ==========================================================
  // AH: Detectar my ethertype y obtener la palabra
  
    // definir ethertyppe DEFINE
  if (type==2064) // 0x0810 : Aparentemente no esta en uso (VLAN)
  {
    // Obtengo el buffer y su tamanho (#bytes)
    Buffer buf_cp = p->GetBuffer();
    uint32_t n_bytes_buffer = p->GetBuffer().GetSize();
    
    NS_LOG_DEBUG ("--> AH: Ethertype de configuracion detectado, " 
      << "#caracteres: " << n_bytes_buffer );
    
    // Creo un char * y le asigno memoria
    char * command;
    command = (char*)malloc((int)n_bytes_buffer);
    
    // Paso el char * como referencia para obtener el payload deserializado
    buf_cp.DeserializeRadioConfigurationPacket(command);

    NS_LOG_DEBUG ("--> AH: Se recibio el comando: " << command);
    
    // Aqui llamaria mi logica para reconfigurar el radio
    int deviceId = (int) m_bridgedDevice->GetIfIndex();
    
    gira::ApiWifiNetDevice apiwifi; // definir const para evitar que se
    // vuelva a recrear
    //NS_LOG_UNCOND ("Este es el sock original: "<< m_sock);
    apiwifi.CallApiWifi(command,(int)m_nodeId, deviceId, m_sock,
        Mac48Address::ConvertFrom (src), Mac48Address::ConvertFrom (dst));
    std::free(command);
    // En este caso el paquete ya no debe continuar su camino.
    return;
  }
  // ==========================================================
...
TapBridge::Filter (Ptr<Packet> p, Address *src, Address *dst, uint16_t *type)
{
  ...
  //NS_LOG_UNCOND ("--> AH: Packet filter: pktSize = " << pktSize
